...
public class TwoAppl implements BrokerStatusListener {
	...
	public TwoAppl(boolean flag){
		igor = new PubSubClient("localhost", 8083);
//		PubSubClient debora = new PubSubClient("localhost", 8083);
//		PubSubClient jonata = new PubSubClient("localhost", 8084);
		
		igor.subscribe("localhost", 8080);
		Thread accessOne = new ThreadWrapper(igor, "igor_acquire_x1", "localhost", brokerPort);
...
//		}
		...
		ActionListener ac = event -> {
			...
			Thread a = new ThreadWrapper(client, "igor".concat("_release_").concat(var), "localhost", 8080);
			a.start();
			try{
				a.join();
			} catch (Exception e){
			...
			Thread accessOne = new ThreadWrapper(client, "igor_acquire_x1", "localhost", 8080);
			accessOne.start();
			try {
				accessOne.join();
			} catch (Exception e) {
	...
	public void onBrokerDown() {
		...
		Thread accessOne = new ThreadWrapper(igor, "access igor- var X", "localhost", brokerPort);
		accessOne.start();
		try {
			accessOne.join();
		} catch (Exception e) {
